...
#include "gta-reversed/game_sa/Enums/eSoundBank.h"
#include "gta-reversed/game_sa/Enums/eSoundBankSlot.h"
...
struct CAEBankSlot {
    ...
    size_t GetSoundSize(tSoundID id) const {
        assert(IsSingleSound() || id >= 0 && id < (tSoundID)(NumSounds));
...
protected:
    ...
    std::array<tSoundID, 60>        m_BankSlotSound; //!< Sound ID if a bank is used for a single sound only, otherwise (when a whole bank is loaded) `-1`
};
