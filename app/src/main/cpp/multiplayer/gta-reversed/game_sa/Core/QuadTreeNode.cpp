/* เพิ่มจาก sasamp-main: CQuadTreeNode */
...
{
    /*
    รอ MAP 2.11: CQuadTreeNode::InjectHooks
    CHook::Write(g_libGTASA + 0x83A410, &ms_pQuadTreeNodePool);
    */
}
...
CQuadTreeNode::CQuadTreeNode(const CRect& size, int32 startLevel) :
    m_ItemList(),
...
int32 CQuadTreeNode::FindSector(const CVector2D& posn)
{
    ...
    if (bLeftHalf) {
        return center.y <= posn.y ? 0 : 2;
    }

    return center.y <= posn.y ? 1 : 3;
}
...
bool CQuadTreeNode::InSector(const CRect& rect, int32 sector) const
{
    ...
    const CRect sectorRect = GetSectorRect(sector);
    return sectorRect.left <= rect.right
...
void CQuadTreeNode::InitPool()
{
    ...
    ms_pQuadTreeNodePool = new CQuadTreeNodePool(500, "QuadTreeNodes");
}
