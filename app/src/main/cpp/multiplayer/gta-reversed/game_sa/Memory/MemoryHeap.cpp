...
#ifdef MEMORY_MGR_USE_HEAP_FLAGS
    assert(!desc->m_Flags.IsBoundBlock && "Cannot free BOUND heap block!");
#endif
...
void* CMemoryHeap::Malloc(uint32 size) {
    ...
    while (blockSize > uint32(block->m_nSize)) {
        ...
        if (block == &m_FreeListEnd) {
            FLog("CMemoryHeap[%d]: !!! failed to allocate %d bytes !!!", m_nMemId, block->m_nSize);
            return nullptr;
        }
    }
...
void CMemoryHeap::PrintMemoryLeaks() {
    ...
    FLog("MemoryHeap %p LEAKS (used %d bytes out of %d bytes total)", m_FirstBlock, m_nMemUsed, GetHeapSize());
    FLog("-----------------------------------------------------------------------------------------------------------");
...
void CMemoryHeap::IntegrityCheck() {
    CHook::CallFunction<void>(g_libGTASA + 0x37524C, this);
}
...
void CMemoryHeap::_DumpHeapStatus(bool bCommonSizes) {
    FLog("MemoryHeap %p STATUS (used %d bytes out of %d bytes total)", m_FirstBlock, m_nMemUsed, GetHeapSize());
    ...
    FLog(" Percentage used:    %.1f", 100.f * float(m_nMemUsed) / float(GetHeapSize()));
    FLog(" Largest free block: %d bytes", GetLargestFreeBlock());
    FLog(" Holes size:         %d bytes", GetSizeOfHoles());
}
